...
import { FactorApi, FactorItemApi, ProfileApi } from "features/repositories";
...
export default function PrintInvoice(props) {
  ...
  React.useEffect(() => {
    FactorApi.GetById(factorId)
      ...
      .then((res) => {
        setFactor(res.data);
        ProfileApi.GetById(res.data.customerId)
          .apply()
          .then((profile) => setCustomer(profile.data));
      });

    FactorItemApi.GetForPrint({ factorId: factorId })
      .apply()
      .then((res) => {
        console.log(res.data);
        setFactorItems(res.data);
        setSumPrice(
          res.data.reduce(
    ...
    <div ref={printRef} id="main" class="invoice-print p-5">
      <div class="d-flex justify-content-between flex-row">
        <div class="mb-4">
          ...
          <p class="mb-1">تبریز، خبایان مطهری</p>
          <p class="mb-1">ساختمان سی تی زن، طبقه همکف، خدمات پس از فروش</p>
          <p class="mb-0">
        ...
        <div>
          <h4>فاکتور #{factor ? factor.number : ""}</h4>
          <div class="mb-2">
            <span>تاریخ پیش فاکتور: </span>
            <span class="fw-semibold">
              {factor
                ? moment(factor.orderDate).locale("fa").format("YYYY/MM/DD")
                : ""}
            </span>
          </div>
          <div>
            <span>تاریخ تائید: </span>
            <span class="fw-semibold">
              {factor
                ? moment(factor.date).locale("fa").format("YYYY/MM/DD")
                : ""}
            </span>
          </div>
        </div>
      ...
      <div class="row d-flex justify-content-between mb-4">
        <div class="col-sm-6 w-50">
          ...
          <p class="mb-1">{customer ? customer.mobile : ""}</p>
          <p class="mb-1">{customer ? customer.address : ""}</p>
          <p class="mb-1">{customer ? customer.tel : ""}</p>
        </div>
        {/*<div class="col-sm-6 w-50">
          <h6>قبض به:</h6>
          <table class="lh-2">
        ...
        <table class="table border-top m-0">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td class="px-4 py-3">
                <p class="fw-semibold mb-2 ">{sumPrice} تومان</p>
                <p class="fw-semibold mb-2">
                  {factor ? (sumPrice * factor.discount) / 100 : 0} تومان
                </p>
                <p class="fw-semibold mb-0">
                  {factor ? sumPrice - (sumPrice * factor.discount) / 100 : 0}{" "}
                  تومان
                </p>
              </td>
            </tr>
      ...
      <div class="row">
        <div class="col-12 lh-1-85 mt-3">
          ...
          {<span>{factor ? factor.comment : ""}</span>}
        </div>
      </div>
    </div>
  );
